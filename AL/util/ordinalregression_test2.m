function [test_result,accuracy_te,abcd_te]=ordinalregression_test2(te,te_label,model)
B=model.B;
...
for i=1:size(te,1)
    pos=find(abcd_te(i,:)==max(abcd_te(i,:)));
   test_result(i,1)=study_range(pos(1));
end
accuracy_te=length(find((test_result-te_label)==0))/length(test_result);
